    ...
    getContext() {
      wx.createSelectorQuery()
        ...
        .context(function(res) {
          const context = res.context as MPNS.VideoContext
          context.seek(0)
        })
        .exec()
    },
  ...
  Page({
    getNode() {
      wx.createSelectorQuery()
        .select('.canvas')
        .node(function(res) {
          const canvas = res.node as MPNS.Canvas
          canvas
        })
        .exec()
    },
...
// Test case from `VideoContext`
{
  ...
  Page({
    videoContext: {} as MPNS.VideoContext,
    onReady() {
...
// Test case from `wx.openSetting`
{
  wx.openSetting({
    success(res) {
      expectType<MPNS.AuthSetting>(res.authSetting)
      res.authSetting = {
        'scope.userInfo': true,
        'scope.userLocation': true,
      }
...
// Test case from `EditorContext.insertImage`
{
  wx.createSelectorQuery()
    .select('#editor')
    .context(res => {
      const editorCtx = res.context as MPNS.EditorContext
