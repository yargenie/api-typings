...
wx.addCard({
  ...
  success(res) {
    expectType<MPNS.AddCardResponseInfo[]>(res.cardList)
  },
})
...
wx.chooseMessageFile({
  ...
  type: 'image',
  success(res) {
    expectType<MPNS.ChooseFile[]>(res.tempFiles)
  },
})
...
wx.request({
  url: '',
  success(res) {
    const data: MPNS.IAnyObject = { res }
...
// https://github.com/wechat-miniprogram/api-typings/issues/82
{
  interface IDialogMethod
    extends Partial<MPNS.Component.MethodOption> {
    f?: () => string
    g: () => void
  }
  type Dialog = MPNS.Component.Instance<{}, {}, IDialogMethod>
  Page({
    f() {
...
import WX = MPNS
{
  ...
  expectType<MPNS.LoginSuccessCallbackResult>(t)
}
