...
const app = getApp<{
  ...
  userInfoReadyCallback(userInfo: MPNS.UserInfo): void;
}>()
...
Page({
  ...
  onLoad(options) {
    ...
    const app = getApp<{
      globalData: { userInfo: MPNS.UserInfo };
    }>()
  ...
  viewTap() {
    ...
    const p = getCurrentPages()[1] as MPNS.Page.Instance<
      { a: number },
      { customData: { b: number } }
    >
...
Page({
  ...
  jump() {
    ...
    query.select('#a').boundingClientRect(res => {
      expectType<MPNS.BoundingClientRectCallbackResult>(res)
    })
    query.selectViewport().scrollOffset(res => {
      expectType<MPNS.ScrollOffsetCallbackResult>(res)
    })
    query.exec(res => {
      expectType<any>(res)
    })
